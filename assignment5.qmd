...
```{r}
fviz_nbclust(hclust, kmeans, method = "wss")
...
```{r}
km_1 <- kmeans(hclust, 2, nstart = 20)
km_1

kplot <- ggplot(data = hclust, aes(x = hh_income, y = income, color = as.factor(km_1$cluster))) +
  geom_point(size = 4) +
  theme_minimal()
kplot
...
```{r}
hclust$cluster <- km_1$cluster

hclust %>%
  filter(cluster==1) %>%
  summary(pop, hh_income, income)
